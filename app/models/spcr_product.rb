class SpcrProduct < ActiveRecord::Base
  ...
  scope :reaction_pass_for, lambda { |user| joins(:spcr).where(:quality_control_id => QualityControl.find_by_process_and_name(Spcr.to_s,:pass), :spcrs => {:user_id => user.id} ) }
  
  scope :reaction_fail, where(:quality_control_id => QualityControl.find_by_process_and_name(Spcr.to_s,:fail))
  scope :reaction_fail_for, lambda { |user_id| joins(:spcr).where(:quality_control_id => QualityControl.find_by_process_and_name(Spcr.to_s,:fail), :spcrs => {:user_id => user.id} ) }
  ...
  scope :qc_pass_for, lambda { |user| joins(:spcr,:pcr_gel_lanes).where(:pcr_gel_lanes => {:quality_control_id => QualityControl.find_by_process_and_name(PcrGelLane.to_s,:pass).id}, :spcrs => {:user_id => user.id} )}
  ...
  scope :qc_fail_for, lambda { |user| joins(:spcr,:pcr_gel_lanes).where(:pcr_gel_lanes => {:quality_control_id => QualityControl.find_by_process_and_name(PcrGelLane.to_s,:fail).id}, :spcrs => {:user_id => user.id} )}
